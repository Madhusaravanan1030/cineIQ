...
// Retry helper — handles ECONNRESET from TMDB free tier
...
router.get("/search", async (req, res) => {
  try {
    ...
    const response = await withRetry(() =>
      tmdb.get("/search/movie", { params: { query, page: 1 } })
    );
    const movies = response.data.results
      .filter((m) => m.poster_path)
      .slice(0, 20)
      .map(formatMovie);
    res.json({ movies, total: response.data.total_results });
  } catch (err) {
...
//  Query TMDB's discover endpoint directly.
//  This searches the ENTIRE TMDB database
//  (900,000+ movies) not just the 20 trending ones.
//
//  Query params supported:
//    year       = 2023           (exact release year)
//    genre      = 28             (TMDB genre ID)
//    language   = en             (original_language code)
//    runtime_max= 119            (max runtime in minutes)
//    runtime_min= 120            (min runtime in minutes)
...
router.get("/discover", async (req, res) => {
  try {
    ...
    // Only include a param if the user actually provided it
    const params = {
      sort_by,
      page,
      "vote_count.gte": 50,       // exclude obscure movies with <50 votes
    };
    ...
    const movies = response.data.results
      .filter((m) => m.poster_path)   // only movies with a poster
      .map(formatMovie);
...
//  Frontend uses this to map genre name → ID.
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  GET /api/movies/:id
//  Full movie detail with cast + director
// ─────────────────────────────────────────────
